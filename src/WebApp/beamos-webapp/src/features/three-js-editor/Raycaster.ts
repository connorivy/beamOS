...
export class Raycaster {
  ...
  raycast() {
    ...
    const intersects = this.raycaster
      ...
      .map(o => o.object as unknown as IBeamOsMesh)

    if (intersects.length > 0) {
      this.tabIndex = intersects.length >= this.tabIndex ? 0 : this.tabIndex
      ...
      if (this.raycastInfo.currentlyRaycasted) {
        if (this.raycastInfo.currentlyRaycasted.id == intersectedObj.id) {
      ...
      intersectedObj.material = this.getHoverMaterial(intersectedObj.material)
    } else {
      this.unhighlightRaycasted()
    }
  }
  ...
  static GetMaterialCloneWithProvidedColor<T extends THREE.Material>(
    ...
    if (material instanceof THREE.MeshStandardMaterial) {
      const clone = material.clone() as T & THREE.MeshStandardMaterial
      clone.emissive.setHex(hex)
      return clone
    }
    if (material instanceof LineMaterial) {
      const clone = material.clone() as T & LineMaterial
