...
import type { FilterStack } from "./FilterStack"
...
export class ModelProposalDisplayer {
  ...
  public displayModelProposal(body: ModelProposalResponse): Promise<Result> {
    ...
    for (const node of body.createNodeProposals ?? []) {
      ...
      filterer.add(newNode, this.config.createNodeProposalHex, false, true)
    ...
    for (const el of body.createInternalNodeProposals ?? []) {
      ...
      const existingElement1d = this.getObjectByBeamOsUniqueId<BeamOsElement1d>(
    ...
    for (const node of body.modifyNodeProposals ?? []) {
      ...
      filterer.add(newNode, this.config.modifyNodeProposalHexNew, false, true)
    ...
    for (const el of body.createElement1dProposals ?? []) {
      ...
      if (!el.id) {
        // todo: not sure this should throw in this case
        throw new Error("Element1dProposal id is undefined")
      }

      const newElement1d = new BeamOsElement1dProposal(
        undefined,
        el.id,
        startNode,
        endNode,
  ...
  private displayEntityAsRemoved(
    ...
    if (el.objectType == BeamOsObjectType._3) {
      ...
      filterer.add(existingNode, this.config.removeNodeProposalHex, false, true)
    }
  }
