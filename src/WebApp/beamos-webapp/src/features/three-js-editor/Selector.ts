...
import type { RaycastInfo } from "./Raycaster"
...
export class Selector {
  ...
  onMouseDown(event: MouseEvent) {
    ...
    if (this.selectionRectElement) {
      ...
      this.selectionRectElement.style.left = `${left.toString()}px`
      this.selectionRectElement.style.top = `${top.toString()}px`
  ...
  onMouseMove(event: MouseEvent) {
    ...
    if (this.selectionRectElement) {
      ...
      this.selectionRectElement.style.left = `${x.toString()}px`
      this.selectionRectElement.style.top = `${y.toString()}px`
      this.selectionRectElement.style.width = `${w.toString()}px`
      this.selectionRectElement.style.height = `${h.toString()}px`
    }
  }
  ...
  selectClickedObject() {
    if (this.raycastInfo.currentlyRaycasted) {
      ...
      if (!isBeamOsMesh(raycastedMesh)) {
        throw new Error(
          `Unable to get object with id ${this.raycastInfo.currentlyRaycasted.id.toString()} from scene`,
        )
      }
  ...
  private handleDragSelect() {
    ...
    const projectToScreen = (vec3: THREE.Vector3): { x: number; y: number } => {
