...
import {
  ...
  type IApiTokenResponse,
} from "../../../../../../../codeGen/BeamOs.CodeGen.StructuralAnalysisApiClient/IdentityApiClientV1"
...
export const ApiTokenDisplay = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      tokenResponse = await identityApiClient.createApiToken(
        new CreateApiTokenRequest({ name: newName, scopes: newScopes }),
      )
    } catch {
      // toast error
  ...
  return (
    <Box>
                  ...
                  <TableCell align="right">
                    <Tooltip title="Delete">
                      <IconButton
                        color="error"
                        onClick={() => {
                          handleDeleteRequest(token.name)
                        }}
                      >
                        <DeleteIcon />
                      </IconButton>
                  ...
                  >
                    ...
                    <DialogContent>
                      <Typography gutterBottom>
                        Are you sure you want to delete this API token? This
                        action is <b>permanent</b> and cannot be undone.
                      </Typography>
                    </DialogContent>
                    <DialogActions>
      ...
      <Dialog open={createDialogOpen} onClose={handleDialogClose}>
        <DialogTitle>
          {showToken ? "Token Created" : "Create New Token"}
        </DialogTitle>
        <DialogContent>
          {showToken ? (
            <Box sx={{ mt: 2, mb: 2 }}>
              ...
              <Typography color="warning.main" sx={{ mb: 1 }}>
                This is the <b>last time</b> you will see this token. Please
                copy and store it securely.
              </Typography>
              <Typography color="text.secondary" variant="body2">
                Treat this token like a password. Do not share it or expose it
                in public places.
              </Typography>
            </Box>
          ) : (
