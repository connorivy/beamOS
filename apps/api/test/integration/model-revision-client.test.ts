...
describe("model revision integration", () => {
    it("updates materials by material name using case-insensitive matching", async () => {
        ...
        expect(updateRevisionResponse.data?.materials[0]?.name).toBe("A36 Steel Updated");
        expect(updateRevisionResponse.data?.materials[0]?.modulusOfElasticity).toBe(120000);
    });
    ...
    it("creates a model revision from batched entity operations", async () => {
        ...
        expect(deleteRevisionResponse.data.parentRevisionId).toBe(createRevisionResponse.data.id);
    });
    ...
    it("gets a branch model revision built by stacking revisions", async () => {
        ...
        const sectionProfileRev1Id = sectionProfileRev1Response.data.sectionProfiles[0]?.id;
        ...
        const materialRev2Id = materialRev2Response.data.materials.find(
            (material) => material.name === "Material Revision 2",
        )?.id;
        ...
        const sectionProfileRev2Id = sectionProfileRev2Response.data.sectionProfiles.find(
        ...
        expect(getModelRevisionResponse.data.materials.map((material) => material.id)).toEqual(
        ...
        expect(
            getModelRevisionResponse.data.element1ds.map((element1d) => element1d.revisionId),
        ).toEqual(expect.arrayContaining([elementRev1RevisionId, elementRev2RevisionId]));
    ...
    it("creates and returns load entities in model revisions", async () => {
        ...
        expect(
            createLoadsRevisionResponse.data?.loadCombinations[0]?.loadCaseFactors[loadCaseId],
        ).toBe(1.5);
        expect(createLoadsRevisionResponse.data?.pointLoads[0]?.force.forceAlongX).toBe(10);
