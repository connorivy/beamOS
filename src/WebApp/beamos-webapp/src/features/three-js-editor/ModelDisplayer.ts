...
export class ModelDisplayer {
  ...
  public async displayModel(modelResponse: ModelResponse): Promise<Result> {
    ...
    const unprocessedInternalNodes = new Set(modelResponse.internalNodes ?? [])
    let progress = true
    while (
      progress &&
      (unprocessedElement1ds.size > 0 || unprocessedInternalNodes.size > 0)
    ) {
      progress = false
      ...
      for (const internalNode of Array.from(unprocessedInternalNodes)) {
        const element1dObj = this.tryGetObjectByBeamOsUniqueId<BeamOsElement1d>(
          BeamOsElement1d.beamOsObjectType,
          internalNode.element1dId,
        )
        if (element1dObj) {
