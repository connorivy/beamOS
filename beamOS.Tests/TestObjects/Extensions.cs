...
namespace beamOS.Tests.TestObjects
{
  ...
  {
    private static void Round(this Matrix<double> matrix, int numDigits = 3)
    {
      matrix.MapInplace(m =>
      {
    ...
    [Pure]
    public static void AssertAlmostEqual(this Matrix<double> calculatedMatrix, Matrix<double> expectedMatrix, int numDigits = 3)
    {
      ...
      Assert.False(exception is EqualException, MatrixUnequalExceptionMessage(calculated, expected));
    }
    [Pure]
    public static string MatrixUnequalExceptionMessage(Matrix<double> calculatedMatrix, Matrix<double> expectedMatrix)
    {
    ...
    public static void AssertSymmetric(this Matrix<double> matrix, int numDigits = 3)
    {
      ...
      Assert.False(exception is TrueException, MatrixUnsymmetricalExceptionMessage(matrixRounded));
    }
    ...
    public static string MatrixUnsymmetricalExceptionMessage(Matrix<double> matrix)
    {
      ...
      return string.Format(CultureInfo.CurrentCulture, "{1}{0}Matrix:   {2}{0}Diff:    {3}{0}",
        Environment.NewLine,
